...
function solution(io: NodeIO): void {
    const I = io.readLine();
    const format = io.readLine();
    const N = io.readInt();
    const min = io.readInt();
    const max = io.readInt();

    //
    // 달러문자와 비달러문자 개수 파악.
    const formatTokens = format.split(/(\$)/g).slice(1);
    const dollarCount = (function getDollorCount() {
        let count = 0;
    ...
    const nonDollarCount = format.length - dollarCount;

    //
    // N번 수행했을 때, 문자열의 길이
    function getSize(step: number) {
        if (dollarCount === 1) {
            return I.length + nonDollarCount * step;
        } else {
            return (
                I.length * dollarCount ** step +
                (nonDollarCount * (1 - dollarCount ** step)) / (1 - dollarCount)
            );
        }
    }

    //
    // 이분탐색으로 1000000100을 넘지않는 마지막 단계의 번호를 알아낸다.
    let lastStep = 0;
    {
        let s = 0;
        let e = N + 1;
    ...
    lastStep = Math.min(N, lastStep + 1);
    ...
    if (dollarCount === 1) {
        let srt = min - 1;
        let end = Math.min(max, getSize(lastStep));
        if (srt < I.length) {
            result.push(I.slice(srt, end));
            srt = Math.min(I.length, end);
        }
        if (I.length < end) {
            srt -= I.length;
            end -= I.length;
            const constant = formatTokens[1];
            const repeatCount = Math.floor((end - srt) / constant.length);
            const removed = constant.length * repeatCount;
            result.push(constant.repeat(Math.max(0, repeatCount)));
            result.push(constant.slice(srt - removed, end - removed));
        }
    } else {
        let depth = 0;
        ...
        while (0 <= depth) {
            ...
            const { isConstantLeaf, step, srt, end } = queue[depth].shift()!;

            //
            // 포맷 문자열에서 $이 아닌 부분에 도달한 경우.
            if (isConstantLeaf) {
                result.push(formatTokens[step].slice(srt, end));
                continue;
            }
            //
            // I에 도달한 경우.
            ...
            for (let i = 0; i < formatTokens.length; i++) {
                ...
                const isConstant = formatToken !== "$";
                s = e;
                e = e + (isConstant ? formatToken.length : getSize(step - 1));
                const maxS = Math.max(s, srt);
                const minE = Math.min(e, end);
                if (maxS < minE) {
                    const from = maxS - s;
                    const to = minE - s;
                    if (isConstant) {
    ...
    const pad = "-".repeat(Math.max(0, max - min + 1 - len));
    io.write(pad);
    io.flush("");
}
...
class NodeIO {
    ...
    public flush(sep = "\n"): void {
        if (this.debug) {
            ...
            console.log(this.stdout.join(sep));
            this.stdout = [];
        }
    }
}
