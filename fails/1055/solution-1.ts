function solution(io: NodeIO): void {
    ...
    //
    // N번 수행했을 때, 문자열의 길이
    const sizeByStep: number[] = [];
    sizeByStep.push(I.length);
    for (let i = 0; i < N; i++) {
        sizeByStep.push(sizeByStep[i] * dollarCount + nonDollarCount);
        if (1000000100 <= sizeByStep[i + 1]) break;
    }

    const result: string[] = [];
    const cache = new Map<string, string[]>();
    function calc(step: number, srt: number, end: number) {
        const key = `${step},${srt},${end}`;
        if (cache.has(key)) {
            result.push(...cache.get(key)!);
            return;
        }
        const beforeStart = result.length;
        if (step === 0) {
        ...
        for (const formatToken of formatTokens) {
            ...
            s = e;
            e = e + (isConstant ? formatToken.length : sizeByStep[step - 1]);
            ...
            if (maxS < minE) {
                ...
                const to = minE - s;
                if (isConstant) {
                    result.push(formatToken.slice(from, to));
                } else {
                    calc(step - 1, from, to);
                }
            }
        }
        const afterStart = result.length;
        cache.set(key, result.slice(beforeStart, afterStart));
    }
    calc(sizeByStep.length - 1, min - 1, max);
    let len = 0;
    for (const t of result) {
        io.write(t);
        len += t.length;
    }
    const pad = "-".repeat(max - min + 1 - len);
    io.write(pad);
    io.flush("");
}
