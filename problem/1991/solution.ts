function solution(io: NodeIO): void {
    type Fix = "pre" | "in" | "post";
    class Tree {
        v: string;
        l?: Tree;
        r?: Tree;
        constructor(v: string, info: Map<string, [string, string]>) {
            this.v = v;
            const [l, r] = info.get(v)!;
            if (l !== ".") this.l = new Tree(l, info);
            if (r !== ".") this.r = new Tree(r, info);
        }
        traverse(fix: Fix, holder: string[] = []) {
            if (fix === "pre") holder.push(this.v);
            if (this.l) this.l.traverse(fix, holder);
            if (fix === "in") holder.push(this.v);
            if (this.r) this.r.traverse(fix, holder);
            if (fix === "post") holder.push(this.v);
            return holder;
        }
    }
    //
    //
    const N = io.readInt();
    const info = makeArray(N, io.readLine).reduce((map, v) => {
        const key = v[0];
        const val = v.split(" ").splice(1) as [string, string];
        map.set(key, val);
        return map;
    }, new Map<string, [string, string]>());
    const tree = new Tree("A", info);
    (["pre", "in", "post"] as Fix[])
        .map((fix) => tree.traverse(fix).join(""))
        .forEach((ans) => io.write(ans));
}
//
// 입출력 객체
class NodeIO {
