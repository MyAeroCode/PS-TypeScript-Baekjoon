...
function solution(io: NodeIO): void {
    ...
    const uniqueInfosSet: UniqueInfo[][] = [
        [
            { alpha: "z", number: 0 },
            { alpha: "w", number: 2 },
            { alpha: "u", number: 4 },
    ...
    const T = io.readInt();
    for (let t = 1; t <= T; t++) {
        const numberCount = makeArray<Array<number>>(Array, 10, () => 0);
        const alphaCounts = new Map<string, number>();
        const stream = io.readLine().toLowerCase();
        for (const char of stream) {
            alphaCounts.set(char, (alphaCounts.get(char) || 0) + 1);
        }
        for (const uniqueInfos of uniqueInfosSet) {
            for (const uniqueInfo of uniqueInfos) {
                const count = alphaCounts.get(uniqueInfo.alpha);
                if (count !== undefined && 0 < count) {
                    numberCount[uniqueInfo.number] = count;
                    for (const alpha of numbers[uniqueInfo.number]) {
                        alphaCounts.set(alpha, alphaCounts.get(alpha)! - count);
                    }
                }
            }
        }
        const ans = numberCount
            .map((count, number) => `${number}`.repeat(count))
            .join("");
        io.write(`Case #${t}: ${ans}`);
    }
}
