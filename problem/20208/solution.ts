...
function solution(io: NodeIO): void {
    const [N, M, H] = makeArray(Int32Array, 3, io.readInt);
    let hp = M;
    let home: Point;
    let milks: Point[] = [];
    let isVisted: boolean[] = [];
    for (let y = 0; y < N; y++) {
        for (let x = 0; x < N; x++) {
            const v = io.readInt();
            if (v !== 0) {
    ...
    //
    let ans = 0;
    function calcDistance(srt: Point, end: Point): number {
        const dy = Math.abs(srt.y - end.y);
        const dx = Math.abs(srt.x - end.x);
        return dy + dx;
    }
    function dfs(srt: Point, cnt: number) {
        for (let i = 0; i < milks.length; i++) {
            if (isVisted[i]) continue;
            const distance = calcDistance(srt, milks[i]);
            ...
            const homeDistance = calcDistance(milks[i], home);
            if (homeDistance <= hp) {
