function solution(io: NodeIO): void {
    const N = io.readInt();
    const heights = makeArray(Int32Array, N, io.readInt).reverse();
    const deque = new FixedArrayAdaptor(Int32Array, N);
    const answer = makeArray(Int32Array, N);
    for (let thisIdx = 0; thisIdx < N; thisIdx++) {
        const thisHeight = heights[thisIdx];
        while (!deque.isEmpty()) {
            const prevIdx = deque.front()!;
            const prevHeight = heights[deque.front()!];
            if (prevHeight <= thisHeight) {
                answer[prevIdx] = thisIdx;
                deque.shift();
            } else break;
        }
        deque.unshift(thisIdx);
    }
    io.write(
        answer
            .map((v) => (v === 0 ? 0 : N - v))
