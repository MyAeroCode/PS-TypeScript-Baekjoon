function solution(io: NodeIO): void {
    ...
    const revx = Number(X.toString().split("").reverse().join(""));
    const revy = Number(Y.toString().split("").reverse().join(""));
    const revans = Number((revx + revy).toString().split("").reverse().join(""));
    io.write(revans);
}
//
// 입출력 객체
