function solution(io: NodeIO): void {
    const T = io.readInt();
    const Ns = makeArray(Int32Array, T, io.readInt);
    const N_MAX = Math.max(...Ns);
    const MOD = 1_000_000_009;
    //
    // count_x[n] = n을 1,2,3으로 표현하는 방법 중 x로 시작하는 방법의 개수.
    const count1 = makeArray(Int32Array, N_MAX + 1);
    const count2 = makeArray(Int32Array, N_MAX + 1);
    const count3 = makeArray(Int32Array, N_MAX + 1);
    count1[1] = 1;
    count1[2] = 0;
    count1[3] = 1;
    count2[1] = 0;
    ...
    count3[2] = 0;
    count3[3] = 1;
    for (let i = 4; i <= N_MAX; i++) {
        count1[i] = (count2[i - 1] + count3[i - 1]) % MOD;
        count2[i] = (count1[i - 2] + count3[i - 2]) % MOD;
        count3[i] = (count1[i - 3] + count2[i - 3]) % MOD;
    }
    //
    // 각 입력을 한줄씩 출력.
    for (const N of Ns) {
        io.write((count1[N] + count2[N] + count3[N]) % MOD);
    }
}
//
// 입출력 객체
