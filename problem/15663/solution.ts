function solution(io: NodeIO): void {
    const [N, M] = [io.readInt(), io.readInt()];
    const nums = makeArray(N, io.readInt).sort((a, b) => a - b);
    function getCombination(
        remains: Set<number> = new Set(makeArray(N)),
        holder: number[] = [],
        combinations: number[][] = [],
    ): number[][] {
        if (holder.length === M) {
            combinations.push(holder.map((i) => nums[i]));
        } else {
            const remainsSnapshot = [...remains].sort((a, b) => a - b);
            for (const remain of remainsSnapshot) {
                remains.delete(remain);
                holder.push(remain);
                getCombination(remains, holder, combinations);
                holder.pop();
                remains.add(remain);
            }
        }
    ...
    [...new Set(getCombination().map((combination) => combination.join(" ")))].forEach(
        io.write,
    );
}
//
