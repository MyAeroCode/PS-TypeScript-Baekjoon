function solution(io: NodeIO): void {
    const [antenna, eyes] = makeArray(2, io.readInt);
    if (antenna >= 3 && eyes <= 4) io.write("TroyMartian");
    if (antenna <= 6 && eyes >= 2) io.write("VladSaturnian");
    if (antenna <= 2 && eyes <= 3) io.write("GraemeMercurian");
}
//
// 입출력 객체
class NodeIO {
