function solution(io: NodeIO): void {
    ...
    const word = io.readLine();

    if (line.length < word.length) io.write(line);
    else {
        const stack = new FixedArrayAdaptor<string[]>(Array, line.length);

        function tryBoom(): boolean {
            let isContain = true;
            const size = stack.size();
            if (size < word.length) return false;
            for (let i = 0; i < word.length; i++) {
                if (stack.get(size - i - 1) !== word[word.length - i - 1]) {
                    isContain = false;
                    break;
                }
            }
        ...
        const remain = stack.asArray().join("");
        io.write(remain ? remain : "FRULA");
    }
}
