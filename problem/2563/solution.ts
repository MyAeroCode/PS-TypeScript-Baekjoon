function solution(io: NodeIO): void {
    const board = makeArray(100, () => makeArray(100, () => 0));
    makeArray(io.readInt, () => {
        const [ldx, ldy] = makeArray(2, io.readInt);
        for (let dx = 0; dx < 10; dx++) {
            for (let dy = 0; dy < 10; dy++) {
                board[ldy + dy][ldx + dx] = 1;
            }
        }
    });
    io.write(
        board.reduce((prev, row) => {
            return prev + row.reduce((a, b) => a + b, 0);
        }, 0),
    );
}
//
