function solution(io: NodeIO): void {
    ...
    while (T--) {
        const N = io.readInt();
        const M = io.readInt();
        const remainPriority: number[] = [];
        const remainDocs = makeArray(N, (idx) => {
            const thisPriority = io.readInt();
            remainPriority.push(thisPriority);
            return { idx, priority: thisPriority };
        });
        remainPriority.sort((a, b) => b - a);
        //
        //
        let count = 0;
        while (true) {
            const currentDoc = remainDocs.shift()!;
            if (currentDoc.priority === remainPriority[0]) {
                remainPriority.shift();
                count++;
                if (currentDoc.idx === M) break;
            } else {
                remainDocs.push(currentDoc);
            }
        }
        io.write(count);
    }
