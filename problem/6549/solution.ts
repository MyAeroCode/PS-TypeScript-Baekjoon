...
function solution(io: NodeIO): void {
    ...
    while (true) {
        const N = io.readInt();
        if (N === 0) break;
        const heights = makeArray(Int32Array, N + 1, (idx) =>
            idx === N ? 0 : io.readInt(),
        );
        let answer = 0;
        const stack = new FixedArrayAdaptor(Int32Array, N + 1);
        for (let end = 0; end < heights.length; end++) {
            const endHeight = heights[end];
            while (true) {
                if (stack.isEmpty()) break;
                const srt = stack.back()!;
                const srtHeight = heights[srt];
                if (endHeight < srtHeight) {
                    stack.pop();
                    const h = srtHeight;
                    const w = end - collapse(stack.back()! + 1, 0);
                    const area = h * w;
                    if (answer < area) {
                        answer = area;
                    }
