...
function solution(io: NodeIO): void {
    ...
    //
    // 보석 정보
    const jewels = makeArray<Jewel[]>(Array, N, () => ({
        weight: io.readInt(),
        price: io.readInt(),
    ...
    //
    // 가방 정보가 담긴 스킵리스트
    const bags = makeArray<number[]>(Array, K, io.readInt).sort((a, b) => b - a);

    //
    // 가격이 높은 쥬얼을, 최대한 가벼운 가방에 넣는다.
    const pq = new PriorityQueue<Jewel>({
    ...
    let ans = 0;
    while (bags.length) {
        const lightBag = bags.pop()!;
        while (true) {
            const jewel = jewels[jewels.length - 1];
            if (jewel === undefined || lightBag < jewel.weight) {
                if (pq.length !== 0) {
                    const selected = pq.dequeue();
                    ans += selected.price;
                }
                break;
            } else {
