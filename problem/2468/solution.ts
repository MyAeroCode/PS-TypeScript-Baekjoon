function solution(io: NodeIO): void {
    const N = io.readInt();
    const board = makeArray(N, () => makeArray(N, () => io.readInt()));
    const deltas = [
        [+1, 0],
        [-1, 0],
        [0, +1],
    ...
    function calcSafeArea(creteria: number): number {
        const group: ([number, number] | undefined)[][] = makeArray(N, () =>
            makeArray(N, () => undefined),
        );
        const groupKeys = new Set<[number, number]>();
        for (let y = 0; y < N; y++) {
            for (let x = 0; x < N; x++) {
                if (group[y][x]) continue;
                if (board[y][x] <= creteria) continue;
                const groupKey = [y, x] as [number, number];
                const queue = [groupKey];
                group[y][x] = groupKey;
                groupKeys.add([y, x]);
                while (queue.length) {
                    const [thisY, thisX] = queue.shift()!;
                    for (const delta of deltas) {
                        const nextY = thisY + delta[0];
                        const nextX = thisX + delta[1];
                        if (nextY < 0 || nextY === N) continue;
                        if (nextX < 0 || nextX === N) continue;
                        if (board[nextY][nextX] <= creteria) continue;
                        if (group[nextY][nextX]) continue;
                        group[nextY][nextX] = groupKey;
    ...
    const ans = Math.max(...makeArray(101, (creteria) => calcSafeArea(creteria)));
    io.write(ans);
}
//
// 입출력 객체
