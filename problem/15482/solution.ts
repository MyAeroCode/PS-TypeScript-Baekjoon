function solution(io: NodeIO): void {
    const a = io.readLine();
    const b = io.readLine();
    const cache = [...Array(a.length + 1).keys()].map(() => Array(b.length + 1));
    for (let j = 0; j <= b.length; j++) cache[0][j] = 0;
    for (let i = 0; i <= a.length; i++) cache[i][0] = 0;
    ...
    io.write(cache[a.length][b.length]);
}
//
// 입출력 객체
class NodeIO {
