class PriorityQueue<T> {
    ...
    pop(): T | undefined {
        if (this.holder.length === 1) return undefined;
        else {
...
class Problem {
    readonly n: number;
    readonly prior = new PriorityQueue<number>((a, b) => a - b);
    readonly later = new PriorityQueue<number>((a, b) => a - b);
...
function solution(io: NodeIO): void {
    const N = io.readInt();
    const M = io.readInt();
    const problems = makeArray<Problem[]>(Array, N + 1, (i) => new Problem(i));

    for (let i = 0; i < M; i++) {
        const prior = io.readInt();
        const later = io.readInt();
        problems[later].prior.push(prior);
        problems[prior].later.push(later);
    }

    const ans: number[] = [];
    const entryProblemIndexs = new PriorityQueue<number>((a, b) => a - b);
    for (let i = 1; i <= N; i++) {
        if (problems[i].prior.size() === 0) {
            entryProblemIndexs.push(i);
        }
    }

    while (entryProblemIndexs.size()) {
        //
        // 선행문제가 없는 문제 중에서, 가장 쉬운문제를 꺼낸다.
        const entryProblemIndex = entryProblemIndexs.pop()!;
        const entryProblem = problems[entryProblemIndex];
        ans.push(entryProblemIndex);

        while (entryProblem.later.size()) {
            //
            // 이 문제의 후행문제들에서,
            // 가장 쉬운 문제를 꺼내어 의존성을 제거한다.
            const nextProblemIndex = entryProblem.later.pop()!;
            const nextProblem = problems[nextProblemIndex];

            //
            // 쉬운 문제부터 풀고 있으므로,
            // 이 문제의 선행문제 중 가장 쉬운 문제는 entryProblem이다.
            nextProblem.prior.pop()!;

            //
            // 의존성 제거 이후, 선행문제의 개수가 0이 되었다면.
            // 이 문제는 새로운 진입점이 된다.
            if (nextProblem.prior.size() === 0) {
                entryProblemIndexs.push(nextProblemIndex);
            }
        }
    }
    io.write(ans.join(" "));
