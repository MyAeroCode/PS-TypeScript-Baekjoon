function solution(io: NodeIO): void {
    const srt = io.readInt();
    const end = io.readInt();
    const size = end - srt + 1;
    const sqrt = Math.sqrt(end);
    const targets = makeArray(Int8Array, size);
    for (let n = 2; n <= sqrt; n++) {
        const squre = n ** 2;
        for (let i = Math.floor(srt / squre); i <= Math.floor(end / squre); i++) {
            targets[squre * i - srt] = 1;
        }
    }
    let cnt = 0;
