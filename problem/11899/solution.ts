function solution(io: NodeIO): void {
    const MAX_SIZE = 50;
    const brokenBracketString = io.readLine();
    const stack = new FixedArrayAdaptor<string[]>(Array, MAX_SIZE + 1);

    let answer = 0;
    for (const bracket of brokenBracketString) {
        const top = stack.back();
        if (top === undefined) {
            if (bracket === ")") answer++;
