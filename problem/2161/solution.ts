function solution(io: NodeIO): void {
    const remains = makeArray(io.readInt, (i) => i + 1).reverse();
    const answer = [] as number[];
    while (1 < remains.length) {
        answer.push(remains.pop()!);
        remains.unshift(remains.pop()!);
    }
    io.write(answer.concat(remains).join(" "));
}
//
// 입출력 객체
class NodeIO {
