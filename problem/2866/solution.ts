function solution(io: NodeIO): void {
    const R = io.readInt();
    const C = io.readInt();
    const words = makeArray<string[]>(Array, R, () => io.readLine());
    const byVerticalAlphas = makeArray<string[][]>(Array, C, () => []);
    for (let i = 0; i < R; i++) {
        const word = words[i];
        for (let j = 0; j < C; j++) {
            byVerticalAlphas[j].push(word[j]);
        }
    }

    const byVertical = byVerticalAlphas.map((v) => v.join(""));
    let count = 0;
    let stop = false;
    while (true) {
        const set = new Set<string>();
        for (let i = 0; i < C; i++) {
            byVertical[i] = byVertical[i].substring(1, byVertical[i].length);
            if (set.has(byVertical[i])) {
                stop = true;
                break;
            } else {
                set.add(byVertical[i]);
            }
        }
        if (stop) {
            break;
