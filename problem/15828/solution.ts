function solution(io: NodeIO): void {
    class Queue {
        ...
        constructor(n: number) {
            this.buffer = new Int32Array(n + 1);
        }
        size(): number {
            const delta = this.rear - this.head;
            if (0 <= delta) return delta;
            else return this.buffer.length + delta;
        }
        insert(n: number) {
            if (this.size() !== this.buffer.length - 1) {
                this.buffer[this.rear++] = n;
                if (this.buffer.length <= this.rear) {
                    this.rear %= this.buffer.length;
                }
            }
        }
        pop(): number | null {
            if (this.size() === 0) return null;
            const poped = this.buffer[this.head++];
            if (this.buffer.length <= this.head) {
        ...
        asArray(): Int32Array {
            ...
            const asArray = new Int32Array(size);
            for (let i = 0; i < size; i++) {
                asArray[i] = this.buffer[(this.head + i) % this.buffer.length];
            }
            return asArray;
        }
    }
    ...
    const result = queue.asArray().join(" ");
