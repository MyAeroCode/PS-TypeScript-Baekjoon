function solution(io: NodeIO): void {
    const gcd = BigInt(io.readInt());
    const lcm = BigInt(io.readInt());
    const mul = gcd * lcm;
    function calcGcd(a: bigint, b: bigint): bigint {
        return b === 0n ? a : calcGcd(b, a % b);
    }
    let ans: BigInt[] = [];
    for (let a = 1n; a * a <= mul; a++) {
        if (mul % a === 0n && calcGcd(a, mul / a) === gcd) {
            ans[0] = a;
            ans[1] = mul / a;
        }
    }
