function solution(io: NodeIO): void {
    ...
    function calcCount(_from: number): number {
        const from = String(_from);
        const to = String(1 - _from);
        const cliped = s.split("");
        if (cliped[0] === from) cliped[0] = "";
        if (cliped[cliped.length - 1] === from) cliped[cliped.length - 1] = "";
        return cliped.filter((v) => v === to).length;
    }

    io.write(Math.min(calcCount(0), calcCount(1)));
}
