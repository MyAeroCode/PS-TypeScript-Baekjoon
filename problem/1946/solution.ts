...
function solution(io: NodeIO): void {
    ...
    while (T--) {
        const N = io.readInt();
        let scores = makeArray<Score[]>(Array, N, () => ({
            doc: io.readInt(),
            itv: io.readInt(),
        }));
        ...
        // 서류등수로 오름차순 정렬했을 때,
        // 면접등수가 내림차순 형태어야 함.
        scores.sort((a, b) => a.doc - b.doc);
        for (let i = 1, creteria = scores[0].itv; i < scores.length; i++) {
            if (scores[i].itv < creteria) {
                creteria = scores[i].itv;
            } else {
                scores[i].itv = Infinity;
                fails++;
            }
        }

        io.write(scores.length - fails);
    }
}
