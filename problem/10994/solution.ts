function solution(io: NodeIO): void {
    ...
    const N = io.readInt();
    const size = (N - 1) * 4 + 1;
    const board = makeArray<Uint16Array[]>(Array, size, () => {
        return makeArray(Uint16Array, size, () => space);
    });
    //
    // 커서를 옮겨가며 각 레벨별로 수행.
    let cursor = 0;
    for (let level = N; 0 < level; level--) {
        if (level === 1) {
            board[cursor][cursor] = star;
        } else {
            const localSize = (level - 1) * 4 + 1;
            //
            // 상하
            for (let nx = cursor; nx < cursor + localSize; nx++) {
                board[cursor][nx] = star;
                board[cursor + localSize - 1][nx] = star;
            }
            //
            // 좌우
            for (let ny = cursor; ny < cursor + localSize; ny++) {
                board[ny][cursor] = star;
                board[ny][cursor + localSize - 1] = star;
            }
        }
        cursor += 2;
    }
    ...
    for (let i = 0; i < size; i++) {
        io.write(String.fromCharCode(...board[i]));
    }
}
//
// 입출력 객체
