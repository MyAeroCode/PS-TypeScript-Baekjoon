...
function solution(io: NodeIO): void {
    const N = io.readInt();
    const lines = makeArray<Line[]>(Array, N, () => ({
        srt: io.readInt(),
        end: io.readInt(),
    })).sort((a, b) => {
        if (a.srt !== b.srt) return a.srt - b.srt;
        return a.end - b.end;
    });

    const clipedLines: Line[] = [lines[0]];
    for (let i = 1; i < lines.length; i++) {
        const thisLine = lines[i];
        const lastLine = clipedLines[clipedLines.length - 1];
        if (thisLine.srt <= lastLine.end) {
            lastLine.end = Math.max(thisLine.end, lastLine.end);
        } else {
            clipedLines.push(thisLine);
        }
    }

    let ans = 0;
    for (const line of clipedLines) {
        ans += line.end - line.srt;
    }
    io.write(ans);
}
