function solution(io: NodeIO): void {
    //
    // a, b를 입력받고 정수부와 소수부로 나눔.
    const a = io.readWord();
    const b = io.readInt();
    const [integer, fraction] = a.split(".");
    //
    // x * 10^y 형태로 재구성.
    const x = BigInt(integer + fraction) ** BigInt(b);
    const y = fraction.length * b;
    //
    // 출력.
    const nStr = x.toString();
    const nFractionLen = nStr.length - y;
    if (nFractionLen <= 0) {
        const part1 = `0.${"0".repeat(nFractionLen * -1)}`;
        const part2 = nStr;
        io.write(`${part1}${part2}`);
    } else {
        const part1 = nStr.substr(0, nFractionLen);
        const part2 = nStr.substr(nFractionLen);
        io.write(`${part1}.${part2}`);
    }
}
//
