function solution(io: NodeIO): void {
    ...
    for (let cnt = 1; cnt <= 25; cnt++) {
        ...
        for (let d = 0; d < N; d++) {
            //
            // 가로
            if (board[d].reduce((a, b) => a + b, 0) === 0) {
                bingo++;
            }
            //
            // 세로
            if (board.map((row) => row[d]).reduce((a, b) => a + b) === 0) {
                bingo++;
            }
        }
        //
        // 좌대각
        if (board.map((_, d) => board[d][d]).reduce((a, b) => a + b) === 0) {
            bingo++;
        }
        //
        // 우대각
        if (board.map((_, d) => board[d][N - d - 1]).reduce((a, b) => a + b) === 0) {
            bingo++;
        }
