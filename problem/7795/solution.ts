function solution(io: NodeIO): void {
    ...
    while (T--) {
        const N = io.readInt();
        const M = io.readInt();
        const A = makeArray(Int32Array, N, io.readInt);
        const B = makeArray(Int32Array, M, io.readInt);
        A.sort((a, b) => a - b);
        B.sort((a, b) => a - b);

        function getUpperBound(target: number): number {
            let s = 0;
            let e = B.length;
            let ans = B.length;
            while (s < e) {
        ...
        const ans = A.map((v) => getUpperBound(v)).reduce((a, b) => a + b);
        io.write(ans);
    }
}
