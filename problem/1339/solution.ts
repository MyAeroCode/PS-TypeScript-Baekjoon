function solution(io: NodeIO): void {
    const N = io.readInt();
    const exps = makeArray<string[]>(Array, N, () => io.readLine());

    //
    // 더해졌을 때, 각 문자에 해당되는 계수를 계산한다.
    const countMap = new Map<string, number>();
    for (const exp of exps) {
        for (let i = 0; i < exp.length; i++) {
            const char = exp[i];
            const weight = 10 ** (exp.length - i);
            if (countMap.has(char) === false) {
                countMap.set(char, weight);
            } else {
                const accWeight = countMap.get(char)!;
                countMap.set(char, accWeight + weight);
            }
        }
    }
    ...
    for (let i = 0; i < counts.length; i++) {
        mapping.set(counts[i].char, 9 - i);
    }

    //
    // 각 문자 표현식을 숫자 표현식으로 바꾼다.
    const convertedExps = exps.map(function convert(expAsStr: string): number {
        let converted = 0;
        for (const char of expAsStr) {
            converted *= 10;
    ...
    const answer = convertedExps.reduce((a, b) => a + b);
    io.write(answer);
}
