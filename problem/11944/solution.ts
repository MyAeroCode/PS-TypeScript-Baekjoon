function solution(io: NodeIO): void {
    const nAsNum = io.readInt();
    const nAsStr = nAsNum.toString();
    const M = io.readInt();
    const len = nAsStr.length;
    if (len * Number(nAsStr) <= M) {
        io.write(nAsStr.repeat(nAsNum));
    } else {
        const q = Math.floor(M / len);
        const r = M % len;
        io.write(nAsStr.repeat(q) + nAsStr.substr(0, r));
    }
}
//
// 입출력 객체
