function solution(io: NodeIO): void {
    const N = io.readInt();
    const expr = io.readLine();
    const operands = new Map<number, number>();
    for (let i = 0; i < N; i++) {
        const operandValue = io.readInt();
        operands.set(65 + i, operandValue);
    }

    const stack = new FixedArrayAdaptor<number[]>(Array, 123);
    for (const token of expr) {
        if (token === "+") {
            ...
            const v = operands.get(token.charCodeAt(0))!;
            stack.push(v);
        }
    }

    const answer = stack.back()!;
    io.write(Number(answer).toFixed(2));
}
