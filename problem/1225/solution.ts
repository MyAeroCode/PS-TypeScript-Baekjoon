function solution(io: NodeIO): void {
    const ans = makeArray<number[]>(Array, 2, () =>
        io
            .readWord()
            .split("")
            .map(Number)
...
// function solution(io: NodeIO): void {
//     const a = io.readWord().split("").map(Number);
//     const b = io.readWord().split("").map(Number);
//     const count = new Int32Array(100);
//     for (const x of a) {
//         for (const y of b) {
//             const key = x * 10 + y;
//             count[key]++;
