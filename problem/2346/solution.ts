function solution(io: NodeIO): void {
    const remains = makeArray(io.readInt(), (idx) => {
        return {
            val: io.readInt(),
            idx: idx + 1,
        };
    ...
    let cur = 0;
    while (true) {
        const poped = remains.splice(cur, 1)[0];
        answers.push(poped.idx);
        if (poped.val < 0) {
            cur += poped.val;
        } else {
            cur += poped.val - 1;
        ...
        if (remains.length <= cur) cur %= remains.length;
        while (cur < 0) cur += remains.length;
    }
    io.write(answers.join(" "));
}
//
// 입출력 객체
class NodeIO {
