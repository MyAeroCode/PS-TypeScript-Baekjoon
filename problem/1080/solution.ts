function solution(io: NodeIO): void {
    const H = io.readInt();
    const W = io.readInt();
    const actual = readBoard();
    const expect = readBoard();
    const diffs = makeArray<Int8Array[]>(Array, H, (y) => {
        return makeArray(Int8Array, W, (x) => {
            return actual[y][x] === expect[y][x] ? 0 : 1;
        });
    });

    //
    // 원하는 형태가 될 때 까지, 왼쪽 위부터 차례대로 뒤집는다.
    let ans = 0;
    for (let y = 0; y < H - 2; y++) {
        for (let x = 0; x < W - 2; x++) {
            if (diffs[y][x] === 1) {
    ...
    //
    // 다 뒤집어도, 원하는 형태가 되지 않았다면,
    // 원하는 형태가 될 수 없는 케이스이다.
    ...
    function readBoard(): Int8Array[] {
        return makeArray<Int8Array[]>(Array, H, () => {
            const line = io.readLine();
            return makeArray(Int8Array, W, (i) => {
                return line[i] === "0" ? 0 : 1;
            });
        });
    }
    function reserve(srtY: number, srtX: number): void {
        for (let y = srtY; y < srtY + 3; y++) {
            for (let x = srtX; x < srtX + 3; x++) {
                diffs[y][x] = diffs[y][x] === 1 ? 0 : 1;
            }
        }
    }
}
