function solution(io: NodeIO): void {
    const infos = makeArray(io.readInt, () => ({
        group: io.readInt(),
        seq: io.readInt(),
        score: io.readInt(),
    })).sort((a, b) => {
    ...
    const rank = [0, 1, 2].map((idx) => infos[idx]);
    if (infos[0].group === infos[1].group && infos[1].group === infos[2].group) {
        rank[2] = infos[3];
    }
    rank.forEach((info) => {
        io.write(`${info.group} ${info.seq}`);
    });
}
//
// 입출력 객체
