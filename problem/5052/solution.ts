function solution(io: NodeIO): void {
    class Trie {
        private data = new Map<string, Trie>();
        private size = 0;

        has([first, ...others]: string[]): boolean {
            if (first === undefined) throw new Error();
            if (others.length === 0) return this.data.has(first);
            return this.of(first).has(others);
        }

        of(elem: string): Trie {
            let childTrie = this.data.get(elem);
            if (childTrie === undefined) {
                childTrie = new Trie();
                this.data.set(elem, childTrie);
            }
            return childTrie;
        }

        insert([first, ...others]: string[]): void {
    ...
    makeArray(io.readInt(), () => {
        ...
        const phoneNums = makeArray(io.readInt(), () => io.readLine())
            .sort()
            .reverse();
        for (const phoneNum of phoneNums) {
            const elems = phoneNum.split("");
            if (trieRoot.has(elems)) {
                return false;
            }
            trieRoot.insert(elems);
        }
        return true;
    })
        .map((result) => (result ? "YES" : "NO"))
        .forEach((v) => io.write(v));
}
//
// 입출력 객체
class NodeIO {
