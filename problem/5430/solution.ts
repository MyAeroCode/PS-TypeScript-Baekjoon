function solution(io: NodeIO): void {
    makeArray(io.readInt(), () => {
        const commands = io.readWord().split("");
        const N = io.readInt();
        const arr = eval(io.readLine()) as number[];
        let l = 0;
        let r = N;
        let isL = true;
        for (const command of commands) {
            if (command === "R") isL = !isL;
            if (command === "D") isL ? ++l : --r;
        }
        if (l <= r) {
            const sliced = isL ? arr.slice(l, r) : arr.slice(l, r).reverse();
            io.write(`[${sliced.join(",")}]`);
        } else {
            io.write("error");
        }
    });
