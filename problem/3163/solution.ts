...
function solution(io: NodeIO): void {
    let T = io.readInt();
    while (T--) {
        const [N, L, K] = makeArray(Int32Array, 3, io.readInt);
        const ants = makeArray<Ant[]>(Array, N, () => ({
            p: io.readInt(),
            id: io.readInt(),
            elapsed: 0,
        }));
        ...
        //
        //
        const l = ants.filter((a) => a.id < 0).map((a) => 0 + a.p);
        const r = ants.filter((a) => a.id > 0).map((a) => L - a.p);
        [...l, ...r].forEach((v, i) => (ants[i].elapsed = v));
