...
function solution(io: NodeIO): void {
    const N = io.readInt();
    const ranges = makeArray<Range[]>(Array, N, () => ({
        srt: io.readInt(),
        end: io.readInt(),
    })).sort((a, b) => {
    ...
    for (let i = 0; i < ranges.length; i++) {
        ...
        while (true) {
            ...
            //
            // 아무것도 없거나, 기존의 스케쥴과 겹치지 않는 경우.
            if (top === undefined || top.end <= cur.srt) {
                accepted.push(cur);
                break;
            }
            //
            // 스케쥴이 겹치지만, 더 적은 시간으로 끝낼 수 있는 경우.
            else if (cur.end < top.end) {
                accepted.pop();
            }
            //
            // 스케쥴이 겹치고, 더 상황이 나빠지는 경우.
            else {
                break;
            }
        }
        ans = Math.max(ans, accepted.length);
    }
    io.write(ans);
}
