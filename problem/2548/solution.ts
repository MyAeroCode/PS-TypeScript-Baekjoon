function solution(io: NodeIO): void {
    const N = io.readInt();
    const array = makeArray(Int32Array, N, io.readInt).sort();
    const acc = new Int32Array(N + 1);
    for (let i = 1; i <= N; i++) {
        acc[i] = acc[i - 1] + array[i - 1];
    }
    const queue = new FixedArrayAdaptor(Int32Array, N);

    let diff = Infinity;
    for (let i = 0; i < array.length; i++) {
        const num = array[i];
        const left = num * i - acc[i];
        const right = acc[N] - acc[i + 1] - num * (N - i - 1);
        const localDiff = left + right;
        if (localDiff < diff) {
            diff = localDiff;
            queue.clear();
            queue.push(num);
    ...
    const candidates = queue.asArray().sort();
    io.write(candidates[0]);
}
