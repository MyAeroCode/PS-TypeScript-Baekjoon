function calcLocalAns(array: Int32Array): number {
    let localAns = 0;
    for (let i = 0; i < array.length; i++) {
        const thisVal = array[i + 0];
        const nextVal = array[i + 1];
        if (nextVal === undefined) {
            localAns += thisVal;
        } else {
            const tied = thisVal * nextVal;
            if (tied <= thisVal) {
                localAns += thisVal;
            } else {
                localAns += thisVal * nextVal;
                i++;
            }
        }
    }
...
function solution(io: NodeIO): void {
    const N = io.readInt();
    const nums = makeArray(Int32Array, N, io.readInt);
    const positives = nums.filter((n) => n > 0).sort((a, b) => b - a);
    const others = nums.filter((n) => n <= 0).sort((a, b) => a - b);
    const ans = calcLocalAns(positives) + calcLocalAns(others);
    io.write(ans);
}
