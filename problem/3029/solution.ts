function solution(io: NodeIO): void {
    const [sh, sm, ss] = io.readLine().split(":").map(Number);
    const [eh, em, es] = io.readLine().split(":").map(Number);
    let s = sh * 3600 + sm * 60 + ss;
    let e = eh * 3600 + em * 60 + es;
    if (e <= s) e += 24 * 3600;
    const diff = e - s;
    const diffs = [diff / 3600, (diff / 60) % 60, diff % 60].map((n) => {
        const floored = Math.floor(n);
        return (floored < 10 ? "0" : "") + floored;
    });
    io.write(diffs.join(":"));
}
//
