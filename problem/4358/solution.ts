function solution(io: NodeIO): void {
    let total = 0;
    let trees = new Map<string, number>();
    while (true) {
        const thisTree = io.readLine();
        if (!thisTree) break;
        total++;
        trees.set(thisTree, (trees.get(thisTree) || 0) + 1);
    }
    [...trees.keys()].sort().forEach((treeName) => {
        const rate = (trees.get(treeName)! * 100) / total;
        io.write([treeName, rate.toFixed(4)].join(" "));
    });
}
//
// 입출력 객체
