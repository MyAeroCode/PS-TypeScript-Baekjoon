function solution(io: NodeIO): void {
    const friendList = new Map<number, number[]>();
    const [N, M] = makeArray(2, io.readInt);
    for (let i = 0; i < N; i++) {
        friendList.set(i, []);
    }
    for (let i = 0; i < M; i++) {
        const [x, y] = makeArray(2, io.readInt);
        friendList.get(x)!.push(y);
        friendList.get(y)!.push(x);
    }
    //
    // 깊이가 5 이상이라면 true.
    let found = false;
    function solve(history: number[]): void {
        if (found || history.length === 5) {
            found = true;
        } else {
            const last = history[history.length - 1];
            for (const next of friendList.get(last)!) {
