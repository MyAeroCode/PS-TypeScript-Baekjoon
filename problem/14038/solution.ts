function solution(io: NodeIO): void {
    const winCnt = makeArray<Array<string>>(Array, 6, io.readWord).reduce(
        (until, cur) => until + Number(cur === "W"),
        0,
    );
    if (5 <= winCnt) io.write(1);
    else if (3 <= winCnt) io.write(2);
    else if (1 <= winCnt) io.write(3);
    else io.write(-1);
}
//
// 입출력 객체
