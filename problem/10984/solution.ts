function solution(io: NodeIO): void {
    makeArray(io.readInt(), () => {
        const grades = makeArray(io.readInt(), () => ({
            credits: io.readInt(),
            grade: Number(io.readWord()),
        }));
        const totalCredits = grades.reduce((untilNow, cur) => {
            return untilNow + cur.credits;
        }, 0);
        const totalGrade = grades.reduce((untilNow, cur) => {
            return untilNow + cur.credits * cur.grade;
        }, 0);
        io.write(`${totalCredits} ${(totalGrade / totalCredits).toFixed(1)}`);
    });
}
//
// 입출력 객체
