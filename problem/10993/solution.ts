function solution(io: NodeIO): void {
    ...
    const space = 32;
    //
    // 주어진 레벨이 차지하는 공간의 크기를 반환
    function getSizeOfLevel(level: number): { w: number; h: number } {
        const acc = (1 << (level - 1)) - 1;
        return {
            w: 1 + (acc << 2),
            h: 1 + (acc << 1),
        };
    ...
    // 입력
    const N = io.readInt();
    const rootSize = getSizeOfLevel(N);
    const board = makeArray<Uint16Array[]>(Array, rootSize.h, () => {
        return makeArray(Uint16Array, rootSize.w, () => space);
    });
    //
    // 맨 좌측 점의 y, x 좌표.
    let cursorY = N & 1 ? rootSize.h - 1 : 0;
    let cursorX = 0;
    //
    // 별찍기.
    for (let level = N; 0 < level; level--) {
        const thisLevelSize = getSizeOfLevel(level);
        const halfW = thisLevelSize.w >> 1;
        const halfH = thisLevelSize.h >> 1;
        //
        // 밑변
        for (let nx = cursorX; nx < cursorX + thisLevelSize.w; nx++) {
            board[cursorY][nx] = star;
        }
        //
        // 양변
        const dy = level & 1 ? -1 : +1;
        for (let step = 0; step <= halfW; step++) {
            const ny = cursorY + dy * step;
            board[ny][cursorX + step] = star;
            board[ny][cursorX + thisLevelSize.w - 1 - step] = star;
        }
        //
        // 맨 좌측 점 갱신
        cursorX += halfH + 1;
