function solution(io: NodeIO): void {
    ...
    const N = io.readInt();
    const K = io.readInt();
    const orders = makeArray(Int32Array, K, io.readInt);

    const using = new Set<number>();
    const posByItem = makeArray<number[][]>(Array, K + 1, () => []);
    for (let pos = K - 1; 0 <= pos; pos--) {
        const item = orders[pos];
        posByItem[item].push(pos);
    }

    for (const order of orders) {
        if (!using.has(order) && using.size === N) {
            let willRemove = 0;
            let willRemoveNextPos = 0;
            for (const item of using) {
                const nextPos = posByItem[item][posByItem[item].length - 1];
                if (nextPos === undefined) {
                    willRemove = item;
                    break;
                }
                if (willRemoveNextPos < nextPos) {
                    willRemove = item;
                    willRemoveNextPos = nextPos;
                }
            }
            using.delete(willRemove);
            using.add(order);
            ans++;
        }
        using.add(order);
