function solution(io: NodeIO): void {
    const N = io.readInt();
    const M = io.readInt();
    const val = makeArray<Int32Array[]>(Array, N, () => {
        return makeArray(Int32Array, N, io.readInt);
    });
    const acc = makeArray<Int32Array[]>(Array, N, () => {
        return makeArray(Int32Array, N, () => 0);
    });
    //
    // 열 누적
    for (let y = 0; y < N; y++) {
        for (let x = 0; x < N; x++) {
            acc[y][x] = (acc[y][x - 1] || 0) + val[y][x];
        }
    }
    //
    // 행 누적
    ...
    // 대답
    for (let i = 0; i < M; i++) {
        const sy = io.readInt() - 1;
        const sx = io.readInt() - 1;
        const ey = io.readInt() - 1;
        const ex = io.readInt() - 1;
        const total = acc[ey][ex];
        const u = 0 < sy ? acc[sy - 1][ex] : 0;
        const l = 0 < sx ? acc[ey][sx - 1] : 0;
        const dup = 0 < sy && 0 < sx ? acc[sy - 1][sx - 1] : 0;
        const ans = total - u - l + dup;
        io.write(ans);
    }
}
