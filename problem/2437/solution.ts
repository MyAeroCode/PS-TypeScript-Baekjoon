function solution(io: NodeIO): void {
    const N = io.readInt();
    const weights = makeArray(Int32Array, N, io.readInt).sort((a, b) => a - b);
    const acc = makeArray(Int32Array, N + 1, () => 0);
    for (let i = 1; i <= weights.length; i++) {
        acc[i] = acc[i - 1] + weights[i - 1];
    }

    let ans = acc[acc.length - 1] + 1;
    for (let i = 1; i < acc.length; i++) {
        if (acc[i - 1] + 2 <= weights[i - 1]) {
            ans = acc[i - 1] + 1;
            break;
        }
    }
