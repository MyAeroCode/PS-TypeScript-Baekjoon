function solution(io: NodeIO): void {
    ...
    function count(val: number): number {
        ...
        const localCount = count(val - 1) + count(val - 2) + count(val - 3);
        cache.set(val, localCount);
        return localCount;
    }
    //
    // 문제풀이.
    const [N, K] = makeArray(Int32Array, 2, io.readInt);
    if (count(N) < K) {
        io.write("-1");
    } else {
        let remainN = N;
        let remainK = K;
        const ans = new FixedArrayAdaptor(Int8Array, N);
        while (remainN) {
            let acc = 0;
            for (let selected = 1; selected <= 3; selected++) {
                const localCount = count(remainN - selected);
                if (remainK <= acc + localCount) {
                    ans.push(selected);
                    remainN -= selected;
                    remainK -= acc;
                    break;
