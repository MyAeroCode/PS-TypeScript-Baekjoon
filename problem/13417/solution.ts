function solution(io: NodeIO): void {
    ...
    class Deque {
        ...
        adjust(idx: number): number {
            return (idx + this.buffer.length) % this.buffer.length;
        }
        //
        // 저장된 요소의 크기
        size(): number {
    ...
    //
    // 문제풀이
    function arrayToString(array: Int32Array): string {
        return String.fromCharCode(...array);
    }
    const TestCaseN = io.readInt();
    for (let _ = 0; _ < TestCaseN; _++) {
        const N = io.readInt();
        const cards = makeArray(N, () => io.readWord().charCodeAt(0));
        const deque = new Deque(N);
        deque.push(cards[0]);
        for (let i = 1; i < cards.length; i++) {
            // io.write(arrayToString(deque.asArray()));
            const first = deque.front()!;
            if (cards[i] <= first) {
                deque.unshift(cards[i]);
            } else {
                deque.push(cards[i]);
            }
        }
